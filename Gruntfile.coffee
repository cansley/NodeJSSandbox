module.exports = (grunt)->
  ...
  grunt.initConfig
    ...
    #   sourceMap: true
      compile:
        files:
          'server.js': 'src/coffee/server/server.coffee'  # 1:1 compile server file
    ...
    exec:
      ...
      notify:
        cmd: 'echo Server started at http://localhost:3000. Activate your LiveReload extension!'
    ...
    #Note: watch doesn't like watching empty dirs. It will throw ambiguous warnings.
    watch:
      coffee:
        files: ['src/coffee/server/*']
        tasks: ['coffee']
        options:
...
#          livereload: true  # default port 35729
  ...
  grunt.registerTask 'default', ['coffee', 'exec:notify', 'concurrent:server_watch']
